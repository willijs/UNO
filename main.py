...
def draw_winner(text):
    ...
    WIN.blit(draw_text, ((int(WIDTH / 2 - draw_text.get_width() / 2)), int(HEIGHT/2 - draw_text.get_height()/2)))
    pygame.display.update()
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_LCTRL and len(yellow_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(yellow.x + yellow.height, yellow.y + yellow.width//2 - 3, 10, 6)
                    yellow_bullets.append(bullet)
                    BULLET_FIRE_SOUND.play()
                
                if event.key == pygame.K_RCTRL and len(red_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(red.x, red.y + red.width//2 - 3, 10, 6)
